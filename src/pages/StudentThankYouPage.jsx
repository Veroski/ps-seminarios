...
export default function StudentThankYouPage() {
  ...
  return <StudentShell><Seo title="Gracias por tu compra | Área de alumnas Patricia Songel" description="Tu compra se está confirmando." canonical={`${studentAreaConfig.appOrigin}${studentAreaConfig.thankYouPath}`} robots="noindex, nofollow" /><div className="student-thanks"><CheckCircle2 size={46} color="#C6A75C" aria-hidden="true" /><p className="student-eyebrow">Pago recibido</p><h1>Ya es <em>tuya.</em></h1><p>Estamos confirmando tu acceso. En cuanto Stripe confirme el pago, verás la formación disponible en tu biblioteca. Puedes dejar esta página abierta unos segundos.</p>{purchasedFormation ? <Link className="student-button student-button--primary" to={studentAreaConfig.coursePath(purchasedFormation.slug)}>Empezar formación <ArrowRight size={16} aria-hidden="true" /></Link> : <Link className="student-button student-button--gold" to={studentAreaConfig.coursesPath}>Ir a mi biblioteca <ArrowRight size={16} aria-hidden="true" /></Link>}</div></StudentShell>;
}
